...
class _RatePlateState extends State<RatePlate> {
  Future<void> rateFood(GraphQLClient client, RatingInput rating) async {
    final MutationOptions _options = MutationOptions(document: gql(r"""
        ...
        """), variables: <String, dynamic>{"rating": rating.toJSON()});

    return await client
        .mutate(_options)
        .then((result) => result.data?["createRating"]);
  }
                    ...
                    StreamButton(
                        ...
                        onClick: () async {
                          ...
                          RatingInput rating = RatingInput(
                              ...
                              publicationId:
                                  this.widget.order.publication?.id ?? '',
                              rate: initialRate.value.toString(),
                              whoRate: databaseService.user.value.id ?? '');
                          this
                              .rateFood(databaseService.client, rating)
                              .then((value) {
                            _streamButtonController
                                .isSuccess()
                                .then((value) async {
