...
class OrderProvider with OrderValidation {
  ...
  BehaviorSubject<DateTime> deliveryDate = new BehaviorSubject<DateTime>.seeded(
      DateTime.now().add(Duration(hours: 3)));
  Stream<DateTime> get deliveryDate$ => deliveryDate.stream;
  Sink<DateTime> get inDeliveryDate => deliveryDate.sink;

  Stream<bool> hasCard(BehaviorSubject<UserDef> user) =>
      user.map((event) => event.allCards?.isEmpty ?? true);
  ...
  Stream<bool> get isAllFilled$ =>
      CombineLatestStream([quantity$], (values) => true).asBroadcastStream();

  Future<OrderInput> validate(
      DatabaseProviderService database, PublicationHome publication) async {
    final total =
        int.parse(publication.pricePerAll ?? "0") * this.quantity.value;
    ...
    final order = OrderInput(
        ...
        fees: fee.toString(),
        totalWithFees: (total + fee).toString(),
        buyerID: database.user.value.id ?? "",
        orderState: "NOT_ACCEPTED",
        sellerId: publication.seller?.id ?? "",
        deliveryDay: deliveryDate.value.toIso8601String(),
        sellerStAccountid: database.user.value.stripeaccountId ?? "",
        paymentMethodAssociated: database.user.value.defaultSource ?? "",
        currency: database.user.value.currency ?? "",
        title: publication.title ?? "",
        adress: database.user.value.adresses!
