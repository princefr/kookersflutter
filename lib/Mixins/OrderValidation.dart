...
mixin OrderValidation {
  ...
  static bool isFuture(DateTime date) =>
      DateTime.now().add(Duration(hours: 3)).isBefore(date);

  final validateQuantity =
  ...
  final validationPaymentMethod =
      StreamTransformer<List<CardModel>, List<CardModel>>.fromHandlers(
          handleData: (value, sink) {
    if (value.isNotEmpty) {
      sink.add(value);
    } else {
